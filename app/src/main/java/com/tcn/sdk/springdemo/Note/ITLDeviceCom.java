...
public class ITLDeviceCom extends Thread implements DeviceSetupListener, DeviceEventListener, DeviceFileUpdateListener, DevicePayoutEventListener {
    ...
    public void run() {
        ...
        while (isrunning) {
            ...
            if (cfg.configUpdate == SSPComsConfig.ComsConfigChangeState.ccNewConfig) {
                ...
                MainNoteActivity.mainActivity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        MainNoteActivity.SetConfig(cfg.baud, cfg.dataBits, cfg.stopBits, cfg.parity, cfg.flowControl);
                    }
                });
    ...
    public void OnNewDeviceSetup(final SSPDevice dev) {
        ...
        MainNoteActivity.mainActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                MainNoteActivity.DisplaySetUp(dev);
            }
        });
    ...
    public void OnDeviceDisconnect(final SSPDevice dev) {
        ...
        MainNoteActivity.mainActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                MainNoteActivity.DeviceDisconnected(dev);
            }
        });
    }
    ...
    public void OnDeviceEvent(final DeviceEvent ev) {
        MainNoteActivity.mainActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                MainNoteActivity.DisplayEvents(ev);
            }
        });
    }
    ...
    public void OnNewPayoutEvent(final SSPPayoutEvent ev) {
        ...
        MainNoteActivity.mainActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                MainNoteActivity.DisplayPayoutEvents(ev);
            }
        });
    ...
    public void OnFileUpdateStatus(final SSPUpdate sspUpdate) {
        ...
        MainNoteActivity.mainActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                MainNoteActivity.UpdateFileDownload(sspUpdate);
            }
        });
