package com.tcn.sdk.springdemo.duitnowkotlin
...
class DuitNowKotlin(
    ...
    private suspend fun callRegisterPayment() {
        try {
            ...
            handleRegistrationError(e.localizedMessage)
        }
    }
            ...
            if (scope.isActive && customDialog?.isShowing == true) {
                ...
                paymentCheckJob = scope.launch(Dispatchers.IO) {
                    try {
                        ...
                        handleQrCodeError(e.localizedMessage + " ")
                    }
                }
            }
        }
    ...
    private fun handleQrCodeError(exception: String) {
        weakActivity.get()?.runOnUiThread {
            if (scope.isActive && customDialog?.isShowing == true) {
                ...
                callback.onLoggingEverything("ERROR: handleQrCodeError: $exception")
    ...
    private fun handleRegistrationError(exception: String) {
        weakActivity.get()?.runOnUiThread {
            if (scope.isActive && customDialog?.isShowing == true) {
                ...
                callback.onLoggingEverything("ERROR: handleRegistrationError: $exception")
    ...
    private suspend fun initDuitNowQueryPaymentChecking(traceNo: String): Result<Unit> {
        ...
        try {
            when (val result = merchantScanDuitNowQueryPaymentChecking(traceNo)) {
                is Result.Success -> {
                    ...
                    when (status) {
                        ...
                        "0" -> {
                            ...
                            return Result.Failure(errorMessage = "Status Unknown")
                        }
                    }
                ...
                is Result.Failure -> {
                    callback.onLoggingEverything("ERROR: Query payment checking failed: ${result.errorMessage}")
    ...
    private suspend fun merchantScanDuitNowQueryPaymentChecking(traceNo: String): Result<JSONObject> {
        return try {
            ...
            Log.d(TAG, "Query SOAP handleTheSoap: $handleTheSoap")
    ...
    private fun startPaymentStatusCheck(traceNo: String) {
        ...
        paymentCheckJob = scope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                initOnLoggingEverything("Step 10: Timeout reached, transaction failed")
                ...
                initOnLoggingEverything("ERROR Exception: Unexpected error in payment status check: ${e.localizedMessage}")
                handleQrCodeError(e.localizedMessage)
            }
        }
    }
    ...
    private fun showErrorDialog(title: String, message: String) {
        weakActivity.get()?.runOnUiThread {
            try {
                ...
                initOnLoggingEverything("ERROR Exception: showing error dialog: ${e.localizedMessage}")
            }
        }
    }
